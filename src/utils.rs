...
// pub(crate) fn post_with_err_handling(url: &str, body: &str, headers: HeaderMap,
//     timeout: Option<Duration>, retries: Option<i32>, delay: Option<Duration>, backoff: Option<i32>) -> Result< serde_json::Value, PostError> {

//     let timeout = timeout.unwrap_or(Duration::from_secs(30));
//     let retries = retries.unwrap_or(3);
//     let delay = delay.unwrap_or(Duration::from_secs(1));
//     let backoff = backoff.unwrap_or(2);
...
//     //     .send()
//     //     .map_err(PostError::NetworkError)?;

//     for _ in 0..retries {
//         if (500..600).contains(&resp.status().as_u16()) {
...
//                 .timeout(timeout)
//                 .send()
//                 .map_err(PostError::NetworkError).unwrap_or(default);
