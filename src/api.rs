///! This module contains the Numerai API wrapper and base Api struct
...
use reqwest::{blocking::{Body, Response}, header::HeaderValue};
...
const API_TOURNAMENT_URL: &str = "https://api-tournament.numer.ai";
...
impl Api {
    /// initialize Numerai API wrapper for Rust
    pub fn new(public_id: Option<&str>, secret_key: Option<&str>,
        verbosity: Verbosity, show_progress: ProgressBar) -> Self {
        let token = match (public_id, secret_key) {
            (Some(public_id), Some(secret_key)) => Some((public_id.to_string(), secret_key.to_string())),
            _ => None,
        };
        let show_progress_bars = match show_progress {
            ProgressBar::Enable => true,
            ProgressBar::Disable => false,
        };
        let tournament_id = 0;
        let global_data_dir = ".".to_string();
        Self {
            token, show_progress_bars, tournament_id, global_data_dir
        }
/*
...
impl Api {
    fn _login(&mut self, public_id: Option<&str>, secret_key: Option<&str>) {
        let public_id = match public_id {
            Some(public_id) => Some(public_id.to_string()),
            None => std::env::var("NUMERAI_PUBLIC_ID").ok(),
        };
        let secret_key = match secret_key {
            Some(secret_key) => Some(secret_key.to_string()),
            None => std::env::var("NUMERAI_SECRET_KEY").ok(),
        };

        // check env variables if not set
        if let (Some(public_id), Some(secret_key)) = (public_id, secret_key) {
            self.token = Some((public_id.to_string(), secret_key.to_string()));
        } else {
            // warn!("You need to supply both a public id and a secret key.");
            println!("You need to supply both a public id and a secret key.");
            self.token = None;
        }
...
*/
    ...
    ///   variables (Option<V>): variables
    ///   authorization (Option<bool>): does the request require authorization (defaults to false)
    ///   retry_options (Option<RetryOptions>):
    ///     retries (i32) for 5XX errors, how often should numerapi retry (defaults to 3)
    ///     delay (i32): in case of retries, how many seconds to wait between tries (defaults to 5)
    ///     backoff (i32): in case of retries, multiplier to increase the delay between retries (defaults to 2)
    /// 
    /// Returns:
    ///   serde_json::Value: Result of the request
    ///
    /// Example:
    /// ```rust
    ///     let query = r#"query($tournament: Int!)
    ///                   {rounds(tournament: $tournament number: 0)
    ///                   {number}}"#;
    ///     let args = json!({"tournament": 1});
    ///     let result = NumerAPI::new().raw_query(query, args);
    /// ```
    fn raw_query<V: serde::Serialize>(self, query: &str, variables: Option<V>,
        authorization: Option<bool>, retry_options: Option<RetryOptions>) -> Result<serde_json::Value, QueryError> {

        let mut headers = reqwest::header::HeaderMap::new();
        headers.insert("Content-type", HeaderValue::from_static("application/json")).expect("cannot insert header");
        headers.insert("Accept", HeaderValue::from_static("application/json")).expect("cannot insert header");
        if let Some(authorization) = authorization {
            if let Some((public_id, secret_key)) = self.token {
                let v = format!("Token {}${}", public_id, secret_key);
                let v = v.parse()
                    .expect("cannot parse token for header");
                headers.insert("Authorization", v)
                    .expect("cannot insert header");
            // } else {
                // return Err(QueryError::RequestError((reqwest::StatusCode::UNAUTHORIZED, "API keys required for this action.")));
            }
        }

        let RetryOptions { retries, delay, backoff } = retry_options.or(Some(RetryOptions::default())).unwrap();
        let variables = match variables {
            Some(variables) => serde_json::json!(variables),
        ...
        // // let body = body.as_object().expect("cannot convert body to object").clone();
        // let body_string = serde_json::to_string(&body)
        //     .expect("cannot convert body to string");

        let client = reqwest::blocking::Client::new();
        let mut delay = delay;
        let mut result = None;
        for _ in 0..retries {
            let res = client.post(API_TOURNAMENT_URL)
                .headers(headers.clone())
                .json(&body)
                .send()
                .map_err(|e|QueryError::ReqwestError(e));
            if res.is_err() || (res.is_ok() && res.as_ref().map(|r|r.status().is_server_error()).unwrap()) {
        ...
        // let client = reqwest::blocking::Client::new();
        // let result = client.post(API_TOURNAMENT_URL)
        //     .headers(headers)
        //     .body(body_string)
        //     .send()
        //     .map_err(|e|QueryError::ReqwestError(e));
        ...
        let result = result.map(|r| {
            ...
            if status.is_success() {
                ...
                Err(QueryError::UnknownError("Server error".into()))
            } else if status.is_client_error() {
                Err(QueryError::UnknownError("Client error".into()))
            } else {
                Err(QueryError::UnknownError("Unknown error".into()))
            }
            // .expect("")
        }).unwrap()?;//.map_err(|e| QueryError::HttpError(e))?;

        Ok(result)

        // if let Some(errors) = result.json::<serde_json::Value>().map_err(QueryError::JsonError)?["errors"].as_array() {
        //     let err = self._handle_call_error(errors);
        //     return Err(QueryError::RequestError((reqwest::StatusCode::BAD_REQUEST, err)));
        // }
        // Ok(result)
...
impl Default for RetryOptions {
    fn default() -> Self {
        RetryOptions { retries: 3, delay: 5, backoff: 2 }
    }
}
